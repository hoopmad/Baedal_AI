...
public class Baedal {
	...
	public static void main(String[] args) {
		...
		double r1 = (Math.round(VeryShortDistance(distance) * 100) / 100.0);
		// 2. 배달 거리가 매우 멀면 배달비가 매우 비싸다.(5000)
		double r2 = (Math.round(VeryLongDistance(distance) * 100) / 100.0);
		// 3. 배달 거리가 매우 멀고 주문 가격이 높지 않으면 배달비가 비싸다.(4000)
		double r3 = (Math.round(Math.min(VeryLongDistance(distance), 1 - HighOrderPrice(price)) * 100) / 100.0);
		// 4. 배달 거리가 가깝고 주문 가격이 보통이면, 배달비가 보통이다.(3000)
		double r4 = (Math.round(Math.min(ShortDistance(distance), MediumOrderPrice(price)) * 100) / 100.0);
		// 5. 배달 거리가 가깝거나 주문 가격이 높으면, 배달비가 싸다.(2000)
		double r5 = (Math.round(Math.max(ShortDistance(distance), HighOrderPrice(price)) * 100) / 100.0);
		// 6. 배달 거리가 멀거나 주문 가격이 낮으면, 배달비가 비싸다.(4000)
		double r6 = (Math.round(Math.max(LongDistance(distance), LowOrderPrice(price)) * 100) / 100.0);
		// 7. 배달 거리가 멀고 주문 가격이 보통이면, 배달비가 보통이다.(3000)
		double r7 = (Math.round(Math.min(LongDistance(distance), MediumOrderPrice(price)) * 100) / 100.0);
		// 8. 배달 거리가 멀고 주문 가격이 높으면, 배달비가 보통이다.(3000)
		double r8 = (Math.round(Math.min(LongDistance(distance), HighOrderPrice(price)) * 100) / 100.0);
